...
int partition_eh_ra()
{
        ...
        print_str( "starting partition eh ra" ); NL;
        NL;
    #endif
    ...
    for( i = 0; i < NUM_PRGS; i++ )
    {
        ...
        for( j = 0; j < NUM_NODES; j++ )
        {
            ...
            if ( tmp < 1 )
            {
                ...
                #ifdef DBG_PARTITION_EH_RA
                    print_str( "load check has passed" ); NL;
                #endif
                ...
                if ( tmp < wccr )
                {
                    ...
                    #ifdef DBG_PARTITION_EH_RA
                        print_str( "energy check has passed" ); NL;
                    #endif
        ...
        {
            #ifdef DBG_PARTITION_EH_RA
                print_str( "partition was not possible" ); NL;
            #endif
            return -1;
        }
...
//----------------------------------------------------------------------
    ...
    void rep_partition()
    {
        ...
        NL;
        print_str( "partition summary" ); NL;
        NL;
...
void create_task_array()
{
    ...
    for ( i = 0; i < NUM_NODES; i++ ) { nodes[ i ].prgs_start_i = 0; }
        ...
        NL;
        print_str( "creating task list/array" ); NL;
    #endif
    ...
    for ( i = 0; i < NUM_NODES; i++ )
    {
            ...
            NL;
            print_str( "finding prgs belonging to n" );
            print_dec( i );
            NL;
        ...
        #ifdef DBG_PRGS_LIST_CREATION
            print_str( "prgs_start_i set to: " );print_dec( nodes[ i ].prgs_start_i );NL;
        #endif
        
        for ( j = 0; j < NUM_PRGS; j++ )
        {
            #ifdef DBG_PRGS_LIST_CREATION
                print_str( "looking at p" );print_dec( j ); NL;
            #endif
            
            if ( prgs[ j ].assigned_to == i )
            {
                #ifdef DBG_PRGS_LIST_CREATION
                    print_str( "found p" );print_dec( j );
