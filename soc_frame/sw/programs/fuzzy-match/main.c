...
#define INT32_MIN (0x00000000)
#define TRUE (1)
#define FALSE (0)
uint32_t fuzzy_match(const char *restrict pattern, const char *restrict str);

static uint32_t compute_score(
		uint32_t jump,
		int first_char,
		const char *restrict match);

static uint32_t fuzzy_match_recurse(
...
uint32_t fuzzy_match(const char *restrict pattern, const char *restrict str)
{
	...
	score += unmatched_letter_penalty * (uint32_t)(slen - plen);
		...
		int first_char)
{
	...
	while ((match = strcasestr(match, search)) != NULL) {
		uint32_t subscore = fuzzy_match_recurse(
				pattern + 1,
				match + 1,
 ...
 */
uint32_t compute_score(uint32_t jump, int first_char, const char *restrict match)
{
...
{
  {
    ...
    {
		  uint32_t score = fuzzy_match(pattern, entries[i]);
		  if (score != INT32_MIN) {
			  Print("  %d|%s\n", PRINTVARS(score, entries[i]));
		  }
	  }
    Print("\n",0);
  }
