...
char *sbc(char *mend, char *shend)
{
	...
	lm = strlen(mend);
	ls = strlen(shend);
	res = (char *)memmgr_alloc(strlen(mend)+1);
	if(res == NULL){
		display_print(0,0, "sbc: Unable to malloc space for result\n");
		//libmin_fail(1);
	}
	p = mnd = (char *)memmgr_alloc(strlen(mend)+1);
	if(mnd == NULL){
		display_print(0,0,"sbc: Unable to malloc\n");
		//libmin_fail(1);
	}
	strcpy(mnd,mend);
...
times_digit(char *mcand, char dgt)
{
	...
	idgt = dgt - '0';
	len = strlen(mcand);
	tmp = (char *)memmgr_alloc(len+2);
	if(tmp==NULL){
		display_print(0, 0, "Unable to malloc partial product array\n");
	//	libmin_fail(1);
	}
...
splice(char *front, char *back)
{
	...
	res = (char *)memmgr_alloc((len+1)*sizeof(char));
	if(res == NULL) {
		display_print(0, 0, "splice: unable to malloc\n");
		return NULL;
	}
	strcpy(res,front);
	strcat(res,back);
...
cut_off(char *source, int n)
{
	...
	k = strlen(source);
	k = ( n > k ? k : n );
	res = (char *)memmgr_calloc(sizeof(char),(k+1));
	if(res == NULL){
		display_print(0, 0, "cut_off: Unable to malloc\n");
		return NULL;
	}
...
my_main(void)
{
	...
	for(i=0; i<strlen(argv[1]); i++)
		if(!isdigit(argv[1][i])){
			Print("%s\n%s\n",PRINTVARS("longdiv: syntax error",
				argv[1]));
			for(j=0;j<i;j++)Putc(' ');
			Putc('^');
			Putc('\n');
	...
	if(j){
		Print("%s\n", PRINTVARS("longdiv: Cannot divide by 0.\n"));
		return FALSE;
	}
	...
	if((ptr = strpbrk(argv[1],DIGITS))==NULL)
	   ...
	   ddnd_len = strlen(ptr);
	ddnds[0] = (char *)memmgr_alloc((ddnd_len+1)*sizeof(char));
	if(ptr == NULL)
		strcpy(ddnds[0],"0");
	else
	        strcpy(ddnds[0],ptr);
	...
	dsr_len = strlen(ptr);
	dsr = (char *)memmgr_alloc((dsr_len+1)*sizeof(char));
	strcpy(dsr,ptr);
	...
	quotient = (char *)memmgr_calloc(sizeof(char),ddnd_len+1);
	...
	*/
	 if(dgts_fwd == 0)
	  ...
	  if(ddnds[step+1]==NULL){
		ddnds[step+1]= memmgr_alloc(2*sizeof(char));
	...
	for(i=0;i<dsr_len+2;i++)Putc(' ');
	for(;*quotient == '0';quotient++)Putc(' ');
	Print("%s\n",PRINTVARS(quotient));

	/* Print out the top bar */
	
	for(i=0;i<dsr_len+2;i++)Putc(' ');   /* space over */
	for(i=0;i<ddnd_len;i++)Putc('_');

	/* Print out divisor and dividend */
	Print("\n %s)%s\n", PRINTVARS(dsr,ddnds[0]));
	...
	for(i=0;i<step;i++){
		...
		for(j=0;j<dsr_len+2+ddnd_len - strlen(ddnds[i]);j++)
			Putc(' ');	
		for(k=0;k<strlen(shends[i]);k++)Putc('-');
		Putc('\n');
