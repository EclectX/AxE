...
int my_main(void)
{
	...
	if(nrows < 0){
		Print("%s: %d is an invalid number of rows\n", PRINTVARS(PROGRAM_NAME,nrows));
		
	}
	if(nrows > MAX_ROWS){
		Print("%s: %d > %d rows maximum\n",PRINTVARS(PROGRAM_NAME,nrows, MAX_ROWS));
	...
	for(i=0;i<nrows;i++){
		...
		for(j=0;j<i+1;j++)
			if( print_centered(triangle[i][j],max_width+2)){
				Print("%s: error printing element %d,%d\n",PRINTVARS(PROGRAM_NAME,i,j));
