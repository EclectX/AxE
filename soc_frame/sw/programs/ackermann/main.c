...
ack(unsigned x, unsigned y)
{
	...
	if (depth > MAX_DEPTH)
  {
		Print("Maximum stack depth %d exceeded. Abort.\n", PRINTVARS(MAX_DEPTH));
		//Fail(1);
	}
	if (x >= MAX_X)
  {
		Print("Maximum x value %d exceeded. Abort. \n", PRINTVARS(MAX_X));
		//Fail(1);
	}
	if (y >= MAX_Y)
  {
		Print("Maximum y value %d exceeded. Abort. \n", PRINTVARS(MAX_Y));
		//Fail(1);
	}
	if (a[x][y])
    return a[x][y];
...
my_main()
{
	...
	for(k=0;k<=AMAX;k++)
  {
    ...
    {
		  depth = 0;  /* stack guard */
			Print("A(%d,%d) = %d\n", PRINTVARS(k-y,y,ack(k-y,y)));
      if (depth > max_depth)
        max_depth = depth;   
		}
	}
  Print("Max recursive depth = %d\n", PRINTVARS(max_depth));
