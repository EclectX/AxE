...
quaternion quat_from_euler(const euler *in_euler)
{
    ...
    if (!in_euler)  // if null
    {
        Print("%s: Invalid input.", PRINTVARS(__func__));
        return out_quat;
    }

    quaternion temp;

    double cy = fp_Cos(in_euler->yaw * 0.5);
    double sy = fp_Sin(in_euler->yaw * 0.5);
    double cp = fp_Cos(in_euler->pitch * 0.5);
    double sp = fp_Sin(in_euler->pitch * 0.5);
    double cr = fp_Cos(in_euler->roll * 0.5);
    double sr = fp_Sin(in_euler->roll * 0.5);
...
euler euler_from_quat(const quaternion *in_quat)
{
    ...
    out_euler.pitch =
        p_Asi(2.0 * (in_quat->w * in_quat->q2 + in_quat->q1 * in_quat->q3));
...
static void test()
{
    ...
    display_print(0, 0, "All tests passed!\n");
}
