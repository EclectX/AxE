...
static const char R1 = 32;  ///< Rotation constant 1 for mixing function G
static const char R2 = 24;  ///< Rotation constant 2 for mixing function G
static const char R3 = 16;  ///< Rotation constant 3 for mixing function G
static const char R4 = 63;  ///< Rotation constant 4 for mixing function G
...
static const char blake2b_sigma[12][16] = {
 ...
 * @returns void
 */
static void G(block_t v, char a, char b, char c, char d, uint64_t x,
              uint64_t y)
{
    v[a] += v[b] + x;
 ...
 */
static int BLAKE2B(char *dest, block_t *d, size_t dd, u128 ll, char kk,
                   char nn)
{
    char bytes[8];
    uint64_t i, j;
 ...
 */
char *blake2b(const char *message, size_t len, const char *key,
                 char kk, char nn)
{
    char *dest = NULL;
    uint64_t long_hold;
    ...
    blocks = memmgr_calloc(dd, sizeof(block_t));
    if (blocks == NULL)
    {
        return NULL;
    }

    dest = memmgr_alloc(nn * sizeof(char));
    if (dest == NULL)
    {
        memmgr_free(blocks);
        return NULL;
 ...
 */
int testnum = 0;
static void assert_bytes(const char *expected, const char *actual,
                         char len)
{
    char i;
    ...
    Print("Test #%d:\n", PRINTVARS(testnum++));
    display_print(0, 0, "Expected[] = { ");
    for (i = 0; i < len; i++)
    {
      Print("0x%02x ", PRINTVARS(actual[i]));
...
static void test()
{
    ...
    display_print(0, 0, "INFO: All tests have successfully passed!\n");
}
